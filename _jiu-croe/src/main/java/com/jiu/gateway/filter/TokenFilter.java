package com.jiu.gateway.filter;
...
import static com.jiu.context.BaseContextConstants.BEARER_HEADER_KEY;
import static com.jiu.exception.code.ExceptionCode.JWT_OFFLINE;

/**
 * @Author Administrator
 * @create 2020/11/13 10:43
 */
@Slf4j
@EnableConfigurationProperties({IgnoreTokenProperties.class})
public abstract class TokenFilter implements WebFilter, Ordered {
    ...
    protected void parseTenant(ServerHttpRequest request, ServerHttpRequest.Builder mutate) {
        // 判断是否忽略tenant
    ...
    protected void parseClient(ServerHttpRequest request, ServerHttpRequest.Builder mutate) {
    ...
    protected Mono<Void> parseToken(ServerWebExchange exchange, WebFilterChain chain) {
        ServerHttpRequest request = exchange.getRequest();
        ServerHttpResponse response = exchange.getResponse();
        ServerHttpRequest.Builder mutate = request.mutate();
        // 判断接口是否需要忽略token验证
