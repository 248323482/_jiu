...
public class BitcoinTransaction {
    ...
    public void signTransaction() throws Exception {
        ...
        for (UTXO output : getOutputs()) {
            if (amount!=-1L&&totalAmount >= (amount + fee)) {
                //余额足够,跳出累加
                break;
            }
        ...
        for (int i = 0; i < getOutputs().size(); i++) {
            ...
            ECKey ecKey;
            if (output.getAddress().equals(ECKey.fromPrivate(prvKey).toAddress(network).toBase58())) {
                ecKey = ECKey.fromPrivate(prvKey);
            } else if (output.getAddress().equals(ECKey.fromPrivate(prvKey, false).toAddress(network).toBase58())) {
                ecKey = ECKey.fromPrivate(prvKey, false);
            } else {
     ...
     * @throws Exception
     */
    public void signTransactions() throws Exception {
        Transaction tran = new Transaction(network);
        //多个私钥
        List<BigInteger> prvKeys = this.prvKeys;
        //总金额
        long totalAmount = 0;
        for (UTXO output : getOutputs()) {
            //-1表示转掉全部
     ...
     * @throws Exception
     */
    public void signSegWitTransaction() throws Exception {
        long totalAmount = 0L;
...
//            stream.write(changeScriptPubKey);
    ...
    public void signSegWitTransactions() throws Exception {
        long totalAmount = 0L;
        boolean hasChange = false;
        for (UTXO output : getOutputs()) {
            totalAmount += output.getAmount();
...
//            stream.write(changeScriptPubKey);
            ...
            for (int i = 0; i < getOutputs().size(); i++) {
                ...
                for (BigInteger prvKey:prvKeys) {
                    try {
                        key = ECKey.fromPrivate(prvKey, true);
                    }catch (Exception e){
    ...
    public static Long getFee(long amount, List<UTXO> utxos) {
        Long feeRate =40L; //((JSONObject)JSONObject.parseObject(HttpUtil.createGet("https://bitcoinfees.earn.com/api/v1/fees/recommended").execute().body())).getLong("fastestFee"); //最新的矿工费率
        Long utxoAmount = 0L;
        Long fee = 0L;
        Long utxoSize = 0L;
        for (UTXO us : utxos) {
