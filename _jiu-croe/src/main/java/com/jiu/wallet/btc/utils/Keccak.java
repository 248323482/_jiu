package com.jiu.wallet.btc.utils;
...
public class Keccak {
    ...
    private void reset(int rateSizebits, int digestSizeBits) {
        ...
        for (int i = 0; i < MAX_STATE_SIZE_WORDS; ++i) state[i] = 0;
        rateBits = 0;
    ...
    private void update(ByteBuffer in) {
        ...
        if (inBytes <= 0) return;
        if (padded) throw new IllegalStateException("Cannot update while padded");
        int rateBits = this.rateBits;
        if ((rateBits & 0x7) > 0) throw new IllegalStateException("Cannot update while in bit-mode");
        long[] state = this.state;
        int rateBytes = rateBits >>> 3;
        int rateBytesWord = rateBytes & 0x7;
        if (rateBytesWord > 0) {
            int c = 8 - rateBytesWord;
            if (c > inBytes) c = inBytes;
            int i = rateBytes >>> 3;
            long w = state[i];
            rateBytes += c;
            inBytes -= c;
    ...
    private void digest(ByteBuffer out) {
        ...
        if (!padded) {
            ...
            if ((rateBits & 0x7) > 0) throw new IllegalStateException("Cannot digest while in bit-mode");
            rateBytes = rateBits >>> 3;
            int rateBytesWord = rateBytes & 0x7;
            if (rateBytesWord > 0) {
                int c = 8 - rateBytesWord;
                if (c > outBytes) c = outBytes;
