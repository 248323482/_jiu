...
//            field.getCustomMap().put("type", type);
            ...
            if (!api.contains("\"")) {
                if (api.contains(".")) {
                    if (api.endsWith("class")) {
                        ...
                        importPackages.add("com.jiu.common.constant.InjectionFieldConstants");
                    }
                } else {
                    importPackages.add(String.format("static com.jiu.common.constant.InjectionFieldConstants.%s", api));
                }
            }
            if (!method.contains("\"")) {
                if (method.contains(".")) {
                    ...
                    importPackages.add(String.format("static com.jiu.common.constant.InjectionFieldConstants.%s", method));
                }
            }
            if (typePackage.contains(".")) {
                importPackages.add(typePackage);
            }
            importPackages.add("com.jiu.injection.annonation.InjectionField");
            importPackages.add("com.jiu.model.RemoteData");
        }
    }
    ...
    private void generateEnum(TableInfo tableInfo, TableField field) throws Exception {
        ...
        String defEnumPackage = entityPackage.replaceAll("entity", "entity.enumeration");
...
//                .append(config.getEntitySuffix()).append(File.separator)
           basePathSb    .append(CodeGenerator.SRC_MAIN_JAVA);
        System.err.println(basePathSb);
