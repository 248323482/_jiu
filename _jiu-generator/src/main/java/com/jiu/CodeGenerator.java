...
import com.jiu.ext.FileOutConfigExt;
import com.jiu.ext.FreemarkerTemplateEngineExt;
import com.jiu.ext.MySqlQueryExt;
import com.jiu.ext.OracleQueryExt;
...
public class CodeGenerator {
    ...
    public static void run(final CodeGeneratorConfig config) {
        ...
        //全局配置
        mpg.setGlobalConfig(globalConfig(config, projectRootPath));
        // 数据源配置
        mpg.setDataSource(dataSourceConfig(config));
        mpg.setPackageInfo(packageConfig(config));
        // 自定义配置
        InjectionConfig cfg = injectionConfig(config);
        mpg.setCfg(cfg);
        // 策略配置
     ...
     */
    private static InjectionConfig injectionConfig(final CodeGeneratorConfig config) {
